...
class uvm_reg_map extends uvm_object;
   ...
   local uvm_reg_map_info   m_reg_arrays_info[uvm_reg_file];
   local uvm_reg_file       m_reg_arrays_by_offset[uvm_reg_addr_t];
   `endif
   ...
   `ifdef UVM_SPARSE_ARRAY
   // Function: add_reg_array
   //
   // Add a register
   //
   // Add the specified register array instance ~rg~ to this address map.
   //
   // The register array is located at the specified address ~offset~ from
   // this maps configured base address.
   //
   // The ~rights~ specify the register array's accessibility via this map.
   // Valid values are "RW", "RO", and "WO". Whether a register field
   // can be read or written depends on both the field's configured access
   // policy (see <uvm_reg_field::configure> and the register's rights in
   // the map being used to access the field. 
   //
   // The number of consecutive physical addresses occupied by the register array
   // depends on the width of the register and the number of bytes in the
   // physical interface corresponding to this address map and the number of
   // registers in the array.
   //
   // If ~unmapped~ is TRUE, the register array does not occupy any
   // physical addresses and the base address is ignored.
   // Unmapped register arrays require a user-defined ~frontdoor~ to be specified.
   //
   // A register array may be added to multiple address maps
   // if it is accessible from multiple physical interfaces.
   // A register array may only be added to an address map whose parent block
   // is the same as the register array's parent block.
   //
   extern virtual function void add_reg_array (uvm_reg_file rg,
                                         uvm_reg_addr_t    offset,
                                         string            rights = "RW",
                                         bit               unmapped=0,
                                         uvm_reg_frontdoor frontdoor=null);
   ...
   extern virtual function void set_reg_map_info (uvm_reg rg,
...
`ifdef UVM_REG_BYTE_ADDRESSING_FIX
   // Function: get_byte_addressing
   //
   // Get the address granularity of the map
   // Returns 1 if the address map was configured using byte-level addressing.
   // Returns 0 otherwise.
   //
   extern virtual function bit get_byte_addressing();
`endif
   ...
   `ifdef UVM_SPARSE_ARRAY
   extern virtual function uvm_reg_map_info get_reg_array_map_info(uvm_reg_file rga,  bit error=1);
   `endif
...
function void uvm_reg_map::add_reg_array(uvm_reg_file rg, 
                                   uvm_reg_addr_t offset,
                                   string rights = "RW",
                                   bit unmapped=0,
                                   uvm_reg_frontdoor frontdoor=null);
   ...
   uvm_reg default_reg;
   if (m_reg_arrays_info.exists(rg)) begin
      `uvm_error("RegModel", {"Register array '",rg.get_name(),
                 "' has already been added to map '",get_name(),"'"})
      return;
   end
   ...
   if (rg.get_parent() != get_parent()) begin
      `uvm_error("RegModel",
         {"Register array '",rg.get_full_name(),"' may not be added to address map '",
          get_full_name(),"' : they are not in the same block"})
      return;
   end
   
   default_reg = rg.get_default_reg();
   default_reg.add_map(this);
...
function void uvm_reg_map::set_reg_map_info(uvm_reg rg,
                                            uvm_reg_map_info info);
...
function bit uvm_reg_map::get_byte_addressing();
   return m_byte_addressing;
endfunction
`endif
...
`ifdef UVM_SPARSE_ARRAY
// get_reg_array_map_info

function uvm_reg_map_info uvm_reg_map::get_reg_array_map_info(uvm_reg_file rga, bit error=1);
  if (!m_reg_arrays_info.exists(rga)) begin
    if (error)
      `uvm_error("REG_NO_MAP",{"Register Array '",rga.get_name(),"' not in map '",get_name(),"'"})
    return null;
  end
  return m_reg_arrays_info[rga];
endfunction
`endif //UVM_SPARSE_ARRAY
...
`ifdef UVM_REG_BYTE_ADDRESSING_FIX
      if (m_byte_addressing)  begin
         if (up_map.get_byte_addressing()==1) begin //moving from byte based to byte based
           if (bus_width < up_map.get_n_bytes(UVM_NO_HIER))  //Scale if going to wider  map
             k = (up_map.get_n_bytes(UVM_NO_HIER) -1)/bus_width + 1;
           else 
             k = 1;
         end else begin //Moving from byte based to  non-byte based
           if ((local_addr.size > 1) && !(upscaling)) begin // if needs multiple physical address then don't scale.
              k = 1;
           end else begin
              if (bus_width <= up_map.get_n_bytes(UVM_NO_HIER))  //Scale if going to wider  map
                 k = 1.0/bus_width;
              else
                 k = real'(up_map.get_n_bytes(UVM_NO_HIER))/bus_width;
           end
         end
      end else begin
         if (up_map.get_byte_addressing()==1) begin //moving from non-byte based to byte based
               if (bus_width < up_map.get_n_bytes(UVM_NO_HIER))
                k = up_map.get_n_bytes(UVM_NO_HIER);
               else
                k = bus_width;
         end else begin // From non-byte based to non-byte based
`endif
            if (bus_width <= up_map.get_n_bytes(UVM_NO_HIER))
               k = 1;
            else 
...
function void uvm_reg_map::Xinit_address_mapX();
   ...
   `ifdef UVM_SPARSE_ARRAY
   foreach (m_reg_arrays_info[rg_]) begin
     uvm_reg_file rg = rg_;
     m_reg_arrays_info[rg].is_initialized=1;
     if (!m_reg_arrays_info[rg].unmapped) begin
//        string rg_acc = rg.Xget_fields_accessX(this);
       uvm_reg_addr_t addrs[];
        
       bus_width = get_physical_addresses(m_reg_arrays_info[rg].offset,0,rg.get_size()*rg.get_default_reg().get_n_bytes(),addrs);
        
       foreach (addrs[i]) begin
         uvm_reg_addr_t addr = addrs[i];

         if (top_map.m_reg_arrays_by_offset.exists(addr)) begin
              `uvm_fatal("RegModel","uvm_reg_map lock on UVM SPARSE array is not handled for this condition");  
//            uvm_reg rg2 = top_map.m_reg_arrays_by_offset[addr];
//            string rg2_acc = rg2.Xget_fields_accessX(this);
//            
//            // If the register at the same address is RO or WO
//            // and this register is WO or RO, this is OK
//            if (rg_acc == "RO" && rg2_acc == "WO") begin
//               top_map.m_reg_arrays_by_offset[addr]    = rg;
//               uvm_reg_read_only_cbs::add(rg);
//               top_map.m_reg_arrays_by_offset_wo[addr] = rg2;
//               uvm_reg_write_only_cbs::add(rg2);
//            end
//            else if (rg_acc == "WO" && rg2_acc == "RO") begin
//               top_map.m_reg_arrays_by_offset_wo[addr] = rg;
//               uvm_reg_write_only_cbs::add(rg);
//               uvm_reg_read_only_cbs::add(rg2);
//            end
//            else begin
...
//               `uvm_warning("RegModel", {"In map '",get_full_name(),"' register '",
//                                         rg.get_full_name(), "' maps to same address as register '",
//                                         top_map.m_reg_arrays_by_offset[addr].get_full_name(),"': 'h",a})
//            end
         end
         else
            top_map.m_reg_arrays_by_offset[addr] = rg;
       ...
       m_reg_arrays_info[rg].addr = addrs;
     end
   end
   `endif //UVM_SPARSE_ARRAY
  ...
  foreach (rw.value[val_idx]) begin: foreach_value
    ...
    foreach(accesses[i]) begin     
      ...
      if (adapter.provides_responses) begin
        ...
        // Integrating the fix for mantis 4820, and restoring original code 
        // using UVM_REG_GET_BASE_RESPONSE_WITHOUT_REQ_ID
        `ifdef UVM_REG_GET_BASE_RESPONSE_WITHOUT_REQ_ID
