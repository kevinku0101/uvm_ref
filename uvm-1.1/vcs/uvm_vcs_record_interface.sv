...
class msglog;
  ...
  // create_msgname
  static function int create_msgname(string name, int stream_id);
    ...
    msgname_aa[cur_msgname_id] = mname;
    return cur_msgname_id;
  endfunction
...
static function void set_recording_detail_file(string filename="");
  ...
  if ($test$plusargs("UVM_TR_RECORD") && !$test$plusargs("UVM_VERDI_TRACE")) begin
    if (filename != "") begin
...
function integer vcs_begin_tr(string txtype,
  ...
  vcs_begin_tr = msglog::create_msgname(nm, stream);  
