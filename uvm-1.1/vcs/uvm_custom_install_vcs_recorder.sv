...
`endif
   ...
   initial begin
     ...
     verdi_tracing = clp.get_arg_matches("+UVM_VERDI_TRACE", verdi_args);
     ...
     if (clp.get_arg_matches("+UVM_LOG_RECORD", tr_args) && (!verdi_tracing && (env_str != "verdi") && !is_sanity_exist)) begin
       vcs_smartlog_catcher _vcs_catcher;
       _vcs_catcher = new();
       uvm_report_cb::add(null,_vcs_catcher);
     end

     // Register the uvm_vcs_recorder to record transactions into VPD
     if (clp.get_arg_matches("+UVM_TR_RECORD", tr_args) && (!verdi_tracing && (env_str != "verdi") && !is_sanity_exist)) begin
       uvm_vcs_recorder vcs_recorder;
       vcs_recorder = new();
       uvm_default_recorder = vcs_recorder;
