...
static int is_reg_hier_dumping = 0; // 9001162946
...
`endif
         if (verdi_clp.is_verdi_trace_ral()) begin //9001175646
...
`ifdef UVM_VERDI_RALWAVE
       if (verdi_clp.is_verdi_trace_ralwave()) begin
           if (is_vcs_uvm_home_exist)
               ...
               pli_inst.dump_class_object_by_file("${VCS_HOME}/etc/uvm-1.1/verdi/register.config"); 
       end
`endif
...
`ifdef VCS
       if (verdi_clp.is_verdi_trace_compwave()) begin
           if (is_vcs_uvm_home_exist)
               ...
               pli_inst.dump_comp_object_by_file("${VCS_HOME}/etc/uvm-1.1/verdi/component.config");
       end
`endif
`endif
     end
...
`else
     ...
     begin
`endif
       static uvm_verdi_recorder verdi_recorder;

       verdi_recorder = new();
       uvm_default_recorder = verdi_recorder;
       pli_inst = uvm_verdi_pli::get_inst();
