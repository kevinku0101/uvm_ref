...
static bit is_uvm_factory_trace_checked = 0;
static bit is_verdi_trace_fac = 0;

function void is_verdi_trace_fac_used(byte sep);
    ...
    foreach (verdi_trace_values[i]) begin
      ...
      foreach (split_values[j]) begin
        case (split_values[j])
          "UVM_AWARE": is_verdi_trace_fac = 1;
          "FAC": is_verdi_trace_fac = 1;
        endcase
      end
    end
endfunction
...
function bit is_uvm_factory_trace();
    if (!is_uvm_factory_trace_checked) begin
        is_verdi_trace_fac_used("|");
        is_verdi_trace_fac_used("+");
        is_uvm_factory_trace_checked = 1;
    end
    return is_verdi_trace_fac;
endfunction
`endif
// End
...
`ifndef UVM_VERDI_NO_FACTORY_RECORDING
  extern function
      void log_factory_info(string label,string original_type_name,string override_type_name,string full_inst_path);
`endif
  // End
...
`ifndef UVM_VERDI_NO_FACTORY_RECORDING
  if (is_uvm_factory_trace())
    log_factory_info("FAC/CREATE",requested_type_name,wrapper.get_type_name(),inst_path);
`endif
  // End
...
`endif
  ...
  // Modified by Verdi
`ifndef UVM_VERDI_NO_FACTORY_RECORDING
  if (is_uvm_factory_trace()) begin
    override_type_name = requested_type.get_type_name();
    if(original_type_name != override_type_name) begin
...
// Modified by Verdi
   // Why is it needed and why is it using $sformat rather than concat
`ifndef UVM_VERDI_NO_FACTORY_RECORDING
function void uvm_factory::log_factory_info(string label,string original_type_name,string override_type_name,string full_inst_path);
  string message;
