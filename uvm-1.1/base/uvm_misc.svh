...
class uvm_status_container;
  ...
  function bit m_do_cycle_check(uvm_object scope);
    //Changed original UVM code uvm_object l = m_uvm_cycle_scopes[$];
    //Change was done to avoid out of bound access error raised by -ntb_obts check
    //We are now only retrieving last element if the size is not 0
