...
`define UVM_IF_METHOD3_BEGIN(method_name,gp,delay,is_nonblocking,phase) \
       ...
       if (obj_size && $cast(__req__,gp)) \
           uvm_default_recorder.tlm2_begin_recording_cb(m_comp,method_name,__req__,delay,is_nonblocking,phase,__r_tr_h__);
`define UVM_IF_METHOD3_END(method_name,gp,delay,is_nonblocking,phase, sync) \
       if (obj_size && $cast(__req__,gp)) \
           uvm_default_recorder.tlm2_end_recording_cb(m_comp,method_name,__req__,delay,__r_tr_h__,is_nonblocking,phase,sync);
`endif
