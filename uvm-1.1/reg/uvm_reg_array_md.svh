...
virtual class uvm_reg_array_md extends uvm_reg_file;
   ...
   local uvm_reg_addr_t    m_size[];
   local uvm_reg_addr_t    m_inter_register_offsets[];
   local uvm_reg_addr_t    m_corrected_offsets[];
   ...
   extern function void     configure  (uvm_reg_block  blk_parent,
                                        ...
                                        int            dimensions,
                                        uvm_reg_addr_t size[],
                                        uvm_reg_addr_t inter_register_offsets[]={},
                                        string         hdl_path = "");
   ...
   //
   extern function void set_inter_register_offsets(uvm_reg_addr_t offset[]);
   ...
   //
   extern virtual function uvm_reg get_reg(uvm_reg_addr_t index[]);
   ...
   //
   // Function: calc_inter_reg_offset
   // Returns the offset of a register based on its index inside the register array
   // User can override this function to provide custom offset calculation algorithm
   //
   extern virtual function uvm_reg_addr_t calc_inter_reg_offset(uvm_reg_addr_t index[],uvm_reg_map map=null);

   //
   // Function: set_hdl_path
   // Returns the hdl path of a register based on its index inside the register array
   // User can override this function to provide custom hdl path based on their implementation
   //
   extern virtual function string set_hdl_path(uvm_reg_addr_t index[]);
   ...
   //
   extern virtual function void set (uvm_reg_addr_t index[],
   ...
   //
   extern virtual function uvm_reg_data_t  get(uvm_reg_addr_t index[],
                                               string         fname = "",
                                               int            lineno = 0);
   ...
   // register.
   //
   extern virtual function bit predict (uvm_reg_addr_t    index[],
                                        uvm_reg_data_t    value,
   ...
   extern virtual function void reset(input uvm_reg_addr_t index[],
                                      string kind = "HARD");
   ...
   // Function: Xget_linear_indexX
   //
   // Return the linear index of the specified index.
   //
   extern local function uvm_reg_addr_t Xget_linear_indexX (uvm_reg_addr_t index[]);
   ...
   //
   extern local function uvm_reg_addr_t             Xcheck_indexX (uvm_reg_addr_t index[],
                                                        string         caller);
   ...
   //
   extern local function void            Xbuild_reg_indexX (uvm_reg_addr_t index,uvm_reg_addr_t index_arr[]);
...
function void uvm_reg_array_md::configure(uvm_reg_block blk_parent, uvm_reg_file regfile_parent, uvm_reg default_reg, int dimensions, uvm_reg_addr_t size[], uvm_reg_addr_t inter_register_offsets[]={}, string hdl_path = "");
...
function void uvm_reg_array_md::set_inter_register_offsets(uvm_reg_addr_t offset[]);
   if(!m_locked) begin
...
function uvm_reg uvm_reg_array_md::get_reg(uvm_reg_addr_t index[]);

    uvm_reg_addr_t linear_index;
...
// calc_inter_reg_offset

function uvm_reg_addr_t uvm_reg_array_md::calc_inter_reg_offset(uvm_reg_addr_t index[],uvm_reg_map map=null);

   uvm_reg_addr_t calculated_offset,multiplier,reg_size,addr_unit_size,min_offset_width,lindex;

   `uvm_info("uvm_reg_array_md",$sformatf("calc_inter_reg_offset called..."),UVM_HIGH)
   reg_size = m_default_reg.get_n_bytes();
   addr_unit_size = map.get_addr_unit_bytes();
   multiplier = (addr_unit_size >= reg_size) ? 1 : (reg_size-1)/addr_unit_size + 1;
   `uvm_info("uvm_reg_array_md",$sformatf("reg_size=%0d, addr_unit_size=%0d, multiplier=%0d, incr=%0p",reg_size,addr_unit_size,multiplier,m_inter_register_offsets),UVM_HIGH)

   calculated_offset = 0;
   if(m_inter_register_offsets.size() == 0) begin
      `uvm_info("uvm_reg_array","inter-register offset not provided",UVM_HIGH)
      lindex = Xget_linear_indexX(index);
      calculated_offset = lindex * multiplier;
   end else begin
      //Use the corrected offsets and no need to check for inaccuracies in offsets provided
      if(this.m_offset_corrected) begin
         //Calculate register offset based on the corrected offsets
         foreach(m_corrected_offsets[i])
            calculated_offset = calculated_offset + index[i] * m_corrected_offsets[i];
         ...
         foreach(m_inter_register_offsets[j]) begin
            ...
            if(m_inter_register_offsets[j] < min_offset_width) begin
               `uvm_info("uvm_reg_array",$sformatf("For map '%s', inter-register offset '%0d' at index location '%0d' in offsets array=%0p ignored as it is found to be smaller than the minimum required size '%0d'. Instead using '%0d' as offset width in place of '%0d'",map.get_name(),m_inter_register_offsets[j],j,m_inter_register_offsets,min_offset_width,min_offset_width,m_inter_register_offsets[j]),UVM_MEDIUM)
               calculated_offset = calculated_offset + index[j] * min_offset_width;
               m_corrected_offsets[j] = min_offset_width;
            end else begin
               `uvm_info("uvm_reg_array",$sformatf("calculated_offset=%0d, index[%0d]=%0d, m_corrected_offsets[%0d]=%0d",calculated_offset,j,index[j],j,m_corrected_offsets[j]),UVM_FULL)
               calculated_offset = calculated_offset + index[j] * m_inter_register_offsets[j];
            end
         end
...
endfunction: calc_inter_reg_offset


// set_hdl_path

function string uvm_reg_array_md::set_hdl_path(uvm_reg_addr_t index[]);

   string hdl_path;
   `uvm_info("uvm_reg_array_md",$sformatf("set_hdl_path called..."),UVM_HIGH)
   hdl_path = "";
   foreach(index[i])
       hdl_path = {hdl_path,$sformatf("[%0d]",index[index.size() - 1 -i])};
   return hdl_path;
endfunction: set_hdl_path
...
function void uvm_reg_array_md::set(uvm_reg_addr_t  index[],
                                    uvm_reg_data_t  value,
                                    string          fname = "",
                                    int             lineno = 0);
...
function uvm_reg_data_t uvm_reg_array_md::get(uvm_reg_addr_t   index[],
                                              string           fname = "",
                                              int              lineno = 0);
...
function bit uvm_reg_array_md::predict (uvm_reg_addr_t    index[],
                                        uvm_reg_data_t    value,
                                        uvm_reg_byte_en_t be = -1,
                                        uvm_predict_e     kind = UVM_PREDICT_DIRECT,
                                        uvm_path_e        path = UVM_FRONTDOOR,
...
function void uvm_reg_array_md::reset(input uvm_reg_addr_t index[], string kind = "HARD");
...
function uvm_reg_addr_t uvm_reg_array_md::Xget_linear_indexX (uvm_reg_addr_t index[]);
   uvm_reg_addr_t linear_index=0;
...
function uvm_reg_addr_t uvm_reg_array_md::Xcheck_indexX (uvm_reg_addr_t index[],
                                              string         caller);
   uvm_reg_addr_t linear_index=0;
...
function void uvm_reg_array_md::Xbuild_reg_indexX (uvm_reg_addr_t index,
                                                   uvm_reg_addr_t index_arr[]);
      uvm_reg_map maps[$];
      uvm_reg_addr_t calculated_offset;
      ...
      for(int i = 0; i < maps.size ; i++) begin
         ...
         calculated_offset = calc_inter_reg_offset(index_arr,maps[i]);
         ...
         bus_width = maps[i].get_physical_addresses(info.offset,0,ral_regs[index].get_n_bytes(),addrs);
         info.addr = addrs;
      ...
      begin
         ...
         foreach(paths[i]) begin
            ...
            foreach(temp_slices[k])
               temp_slices[k].path = {temp_slices[k].path,set_hdl_path(index_arr)};
