...
`ifndef UVM_DISABLE_UNMAPPED_WARNING   
   else begin
      // This is not a root block
      foreach (maps[map_]) begin
         uvm_reg_map map = map_;
         if (map.get_parent_map() == null) begin
            `uvm_warning("REG/MAP/UNMAPD", $sformatf("Address map \"%s\" in block \"%s\" is not mapped in an address map in parent block of type \"%s\". Possibly the sub_block was locked prior to having the block add the sub_blocks map as a submap", map.get_name(), get_full_name(), parent.get_type_name()));
         end
      end
   end
`endif   
...
function uvm_reg_cvr_t uvm_reg_block::build_coverage(uvm_reg_cvr_t models);
   ...
   if (rsrc == null) return UVM_NO_COVERAGE; 
