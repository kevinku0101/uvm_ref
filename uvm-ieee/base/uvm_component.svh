...
`ifdef UVM_VCS_DTL                       
    // allow test component "uvm_test_top" to be created when using save restore flow
    if (bld.get_state() == UVM_PHASE_DONE && !uvm_root::snps_uvm_save_restore_enabled) begin
`else
...
`ifdef UVM_VCS_DTL
  if (parent.has_child(name) && this != parent.get_child(name) && !uvm_root::snps_uvm_save_restore_enabled) begin
`else
...
`ifdef UVM_VCS_DTL                       
  // Allow the new uvm_test_top created to be the child of uvm_top when UVM save restore enabled

  if (m_children.exists(child.get_name()) &&
      m_children[child.get_name()] != child && !uvm_root::snps_uvm_save_restore_enabled) begin
`else
  if (m_children.exists(child.get_name()) &&
...
`ifdef UVM_VCS_DTL                       
  if (m_children_by_handle.exists(child) && !uvm_root::snps_uvm_save_restore_enabled) begin
`else
