...
virtual class uvm_port_component_base extends uvm_component;
  ...
  function new (string name, uvm_component parent);
   ...
   // Modified by Verdi
  pure virtual function void get_provided_to_component(ref uvm_component list[string]);
  //
...
class uvm_port_component #(type PORT=uvm_object) extends uvm_port_component_base;
  ...
  // Modified by Verdi
  virtual function void get_provided_to_component(ref uvm_component list[string]);
    m_port.get_provided_to_component(list);
  endfunction
  //
...
`ifndef UVM_VERDI_NO_HIER_TRACE
    begin
      if (is_verdi_trace_dht_used()) begin
        string label, message, caller_name, provider_name;

        label = {"Connecting ", this.get_name(), " with ", provider.get_name()};
        caller_name = this.get_full_name();
        ...
        `uvm_info("SNPS_PORT_CONN_TRACE",message,UVM_LOW)
      end
    end
`endif
    // End  
